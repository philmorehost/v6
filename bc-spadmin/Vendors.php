            ...
            <tbody>
                    ...
                    if(mysqli_num_rows($get_active_user_details) >= 1){
                        while($user_details = mysqli_fetch_assoc($get_active_user_details)){
                            ...
                            $manage_api_user_account = '<span onclick="manageVendorApi(`'.$user_details["id"].'`, `'.$user_details["email"].'`);" style="text-decoration: underline; color: orange;" class=""><i title="Manage Account" style="" class="bi bi-cart" ></i></span>';
