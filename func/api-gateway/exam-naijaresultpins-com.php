...
if (in_array($product_name, array_keys($exam_service_provider_alter_code))) {
	...
	if (in_array($quantity, array_keys($web_exam_size_array))) {
		$curl_url = "https://sandbox." . $api_detail["api_base_url"] . "/api/v1/exam-card/buy";
		...
		$curl_postfields_data = json_encode(array("card_type_id" => $web_exam_size_array[$epp], "quantity" => 1), true);
