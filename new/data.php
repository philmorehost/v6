                    ...
                    if(mysqli_num_rows($product_discount_table) > 0){
                        while($product_details = mysqli_fetch_assoc($product_discount_table)){
                          if($product_details["val_2"] > 0){
                            $plan_text = strtoupper($network_name) . " " . strtoupper(str_replace("-", " ", $data_type)) . " " . str_replace("_", " ", $product_details["val_1"]) . ' @ N' . $product_details["val_2"] . ' (Validity ' . $product_details["val_3"] . 'days)';
