...
if(isset($_POST["buy-data"])){
    $purchase_method = "web";
    // This is the logic from web/func/data.php
    ...
    if (in_array($purchase_method, $purchase_method_array)) {
        if ($purchase_method === "WEB") {
            $isp = mysqli_real_escape_string($connection_server, trim(strip_tags(strtolower($_POST["network"]))));
            $phone_no = mysqli_real_escape_string($connection_server, trim(strip_tags($_POST["phone"])));
            $type = mysqli_real_escape_string($connection_server, trim(strip_tags(strtolower($_POST["data-type"]))));
            $quantity = mysqli_real_escape_string($connection_server, trim(strip_tags(strtolower($_POST["data-plan"]))));
